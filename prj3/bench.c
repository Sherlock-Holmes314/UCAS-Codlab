...
int main() {
  ...
  for (int i = 0; i < ARR_SIZE(benchmarks); i ++) {
    ...
    if (msg != NULL) {
      ...
      unsigned long instruction_num = ULONG_MAX;
      unsigned long WL_num = ULONG_MAX;
      unsigned long WL_delay = ULONG_MAX;
      int succ = 1;
      for (int i = 0; i < REPEAT; i ++) {
        ...
        if (res.instruction_num < instruction_num) instruction_num = res.instruction_num;
        if (res.WL_num < WL_num) WL_num = res.WL_num;
        if (res.WL_delay < WL_delay) WL_delay = res.WL_delay;
      }
      ...
      printk("Cycles : %u\n",(unsigned int)msec);
      printk("Number of Instruction : %u\n",(unsigned int)instruction_num);
      printk("Number of Write and Load : %u\n",(unsigned int)WL_num);
      printk("Number of Write and Load delay cycles: %u\n",(unsigned int)WL_delay);
